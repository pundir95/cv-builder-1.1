...
const Section = <T,>({
  ...
  return (
    <section id={section.id} className="grid">
      <h4 className={`pb-0.5 text-sm font-bold  ${color ? `text-${color}` : 'group-[.sidebar]:text-primary'}`}>
        {section.name}
      </h4>
      <hr style={{ "height": "1px", "background": color || "#000", "border": "none", "margin": "10px 0px" }} />
              ...
              <div key={item.id} className={cn("space-y-2", className)}>
                ...
                {summary !== undefined && !isEmptyString(summary) && (
                  <div
                    ...
                    className={`wysiwyg ${color ? `!text-${color}` : ''}`}
                  />
                )}
                ...
                {keywords !== undefined && keywords.length > 0 && (
                  <p className={`text-sm ${color ? `text-${color}` : ''}`}>{keywords.join(", ")}</p>
                )}
              </div>
            );
          })}
...
const Experience = () => {
  ...
  return (
    <div className="flex gap-5 flex-col mb-4">
      <Section<ExperienceType> section={section} urlKey="url" summaryKey="summary" color="white">
        {(item) => (
...
const Education = () => {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="inner-part">
        <Section<EducationType> section={section} urlKey="url" summaryKey="summary" color="white">
          {(item) => (
    ...
    <div className="flex gap-5 min-h-[100vh] lg:max-h-none max-h-[100vh] overflow-auto">
      ...
      <div className="w-[40%]">
        <div className="profile-pic mb-6">
          ...
          <Picture className="ml-auto w-full !max-w-[254px] !h-[254px] object-cover rounded-l-[60px] rounded-t-[0px]" />
        </div>
        {isFirstPage && <Summary />}
        {<Skills />}
        {/* <div className="about-container pr-4 mb-5">
