        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-2">
          ...
          <div>
            ...
            <Select value={form.industry} onValueChange={(v: string) => handleChange('industry', v)}>
              <SelectTrigger id="industry" className={errors.industry ? 'border-red-300' : ''}>
          ...
          <div>
            ...
            <input
              ...
              accept=".pdf,.doc,.docx"
              onChange={handleFile}
              className={`block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 ${errors.file ? 'border-red-300 bg-red-50' : ''}`}
            />
            {form.file && <p className="text-green-600 text-xs mt-1">Selected: {form.file.name}</p>}
            {errors.file && <p className="text-red-500 text-xs mt-1">{errors.file}</p>}
          </div>
          <DialogFooter>
            <Button type="submit" loading={submitting} className="w-full mt-2 bg-[#D6EF3C]/90 text-black rounded-full">
              Submit
            </Button>
          </DialogFooter>
        </form>
